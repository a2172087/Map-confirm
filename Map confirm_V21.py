...
class ImageClassifier(QWidget):
    def __init__(self):
        ...
        self.folder_color_mapping = OrderedDict([
            ...
            #Colors Backup
            #('Coral', '#FF7F50'),  # 珊瑚色
            #('SlateBlue', '#6A5ACD'),  # 板岩藍
            #('Gold', '#FFD700'),  # 金色
            #('MediumSeaGreen', '#3CB371'),  # 中海綠色
            #('Crimson', '#DC143C'),  # 深紅色
            #('DarkOrange', '#FF8C00'),  # 深橙色
            #('MediumOrchid', '#BA55D3'),  # 中蘭花紫
            #('SteelBlue', '#4682B4'),  # 鋼青色
            #('Sienna', '#A0522D')  # 赭色


        self.No_folder_color_mapping = {
            '100': 'Surface(Incoming defect)',
            '103': '103_AVI Defect',
            '105': '105_Die Screen Defect',
            '109': '109_FAB IQA Defect',
            '115': '115_Large defect',
            '117': '117_Pad discoloration',
            '126': '126_Die crack',
            '127': '127_Wafer surface scratch',
            '146': '146_Bump defect',
            '201': '201_P/M NO. Out Spec.',
            '203': '203_Probing Void',
            '204': '204_Missing Probe Mark',
            '503': '503_Missing bump',
            '504': '504_Bump residue',
            '505': '505_Irregular bump',
            '506': '506_Bump house defect',
            '507': '507_Bump P/M diameter out of spec.',
            '510': '510_Large bump',
            '521': '521_Small bump',
            '522': '380_special P/M shfit',
        ...
        self.subfolder_priority = {
            '151_Ugly_Die(2D)': 1,
            '101_Wafer Scratch(08)': 2,
            '501_Bump scratch(24)': 3,
            '102_Foreign_material(09)': 4,
            '502_Bump foreign Material(25)': 5,
            '200_Probe_Mark_Shift(10)': 6,
            '500_Bump PM shift(23)': 7,
            '202_PM area out spec.(12)': 8,
            '100_Process_Defect(07)': 9,
            '000_Particle(16)': 10,
            '205_Al_particle_out_of_pad(0F)': 11,
            '999_Al_particle(18)': 12,
            '186_Other(BA)': 13    
        }
    ...
    def performDefectChipSummary(self):
        try:
            ...
            folder_path = QFileDialog.getExistingDirectory(self, '選擇要進行累計Defct count by chip的資料夾路徑,資料夾內必須確保Map Excel都是相同的Group!!')
            if not folder_path:
                return
            ...
            if len(map_workbook.sheetnames) > 1:
                # 提示使用者選擇要做為Format的MAP工作表
                sheet_name, ok = QInputDialog.getItem(self, '選擇Format工作表', '請選擇要做為Format的MAP工作表:', map_workbook.sheetnames, 0, False)
                if not ok:
                    return
            else:
            ...
            # 詢問使用者是否確認執行
            reply = QMessageBox.question(self, '確認執行', f'請注意,除了選擇的工作表"{sheet_name}"以外其餘Sheet將刪除,此過程不可逆,請確認是否執行?', QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes)
    ...
    def save_log(self):
        try:
            ...
            # 建立儲存log的資料夾路徑
            #log_folder = r'M:\Q200\02 QE Personal Data\Racky\Log History\Map confirm'
            log_folder = r'M:\QA_Program_Raw_Data\Log History\Map confirm'
